...
export default function GoodProjFormPage() {
  ...
  const downloadJson = () => exportJson(getValues(), "good-project.json");
  const downloadCsv = () => exportCsv(getValues(), "good-project.csv");
  const downloadPdf = () => exportPdf(getValues(), "good-project.pdf");
  ...
  const handleImport = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        reset({ ...defaultValues, ...parsed }, { keepDefaultValues: true });
      } catch {
  ...
  const handleReset = () => {
    reset(defaultValues);
    setJsonPreview(JSON.stringify(defaultValues, null, 2));
  };
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button type="button" className="btn" onClick={downloadCsv}>Exporter CSV</button>
          <button type="button" className="btn" onClick={downloadPdf}>Exporter PDF</button>
